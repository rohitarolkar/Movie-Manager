class Movie < ActiveRecord::Base
  ...
  def self.fetch_movies(args)
    if args[:search].present?
      where('title ILIKE ?', "%#{args[:search]}%").paginate(:page => args[:page], :per_page => 12)
    else
      #Movie.find(:all,:order => sort_column(args[:sort]) + " " + sort_order(args[:direction]) )
      order(sort_column(args[:sort]) + " " + sort_order(args[:direction])).paginate(:page => args[:page], :per_page => 12)
    end
  end
